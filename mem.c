...
void unmap_for_exit();
void copy_mmap_struct(struct proc_mapping *, const struct proc_mapping *);
...
int mmap(void *addr, int length, int prot, int flags, struct file *f, int offset)
{
    ...
    int index;
    if (p->active_vm_maps == 32)
        return -1;

    if (flags & MAP_FIXED)
    {
        if ((uint)addr < MMAP_BASE)
            return -1;
...
int check_for_add(uint addr, struct proc *p, int length)
{
    ...
    for (i = 0; i < p->active_vm_maps; i++)
    {
        ...
        {
            if (addr + PGROUNDUP(length) > end_add)
                return -1;
            break;
        }
        start_add = PGROUNDUP(p->vm_mappings[i].addr + p->vm_mappings[i].length);
...
void right_shift(struct proc *p, int index)
{
    ...
    while (j >= index)
    {
        copy_mmap_struct(&p->vm_mappings[j + 1], &p->vm_mappings[j]);
        j--;
    }
}
...
int munmap(void *addr, int length)
{
    ...
    {
        copy_mmap_struct(&p->vm_mappings[i], &p->vm_mappings[i + 1]);
        i++;
    }
    p->active_vm_maps--;
    return 0;
...
void unmap_for_exit()
{
    ...
    {
        if (p->vm_mappings[i].flags & MAP_SHARED && p->vm_mappings[i].is_child_process == 1)
            continue;
...
int copy_vmmaps_fork(struct proc *np)
{
    ...
    for (int i = 0; i < p->active_vm_maps; i++)
    {
        ...
        copy_mmap_struct(&np->vm_mappings[i], &p->vm_mappings[i]);
        ...
        else if (np->vm_mappings[i].flags & MAP_SHARED)
        {
            ...
            np->vm_mappings[i].is_child_process = 1;
        }
    }
    return 0;
}

void copy_mmap_struct(struct proc_mapping *dest, const struct proc_mapping *src)
{
    dest->addr = src->addr;
    dest->length = src->length;
    dest->prot = src->prot;
    dest->flags = src->flags;
    dest->is_child_process = src->is_child_process;
